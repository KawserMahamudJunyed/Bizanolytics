...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 backdrop-blur-md border-b border-border/50 bg-background/50">
        <div className="flex items-center">
        <Link href="/">
          <img src="/logo-full.svg" alt="Bizanolytics Logo" className="h-5 sm:h-6 md:h-8 w-auto cursor-pointer" />
        </Link>
        </div>
        <div className="flex items-center gap-3 sm:gap-4">
          <Link href="/login" className="text-[13px] sm:text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
            Log in
          </Link>
          <Link href="/signup" className="text-[13px] sm:text-sm font-medium bg-primary text-primary-foreground px-3 py-1.5 sm:px-4 sm:py-2 rounded-full hover:bg-primary/90 transition-colors">
            Get Started
          </Link>
        </div>
      </nav>
      ...
      <footer className="border-t border-border bg-secondary/30 py-12 px-6 lg:px-8">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center">
          <Link href="/">
            <img src="/logo-full.svg" alt="Bizanolytics" className="h-5 sm:h-6 md:h-8 w-auto opacity-70 grayscale hover:opacity-100 hover:grayscale-0 transition-all cursor-pointer" />
          </Link>
          </div>
